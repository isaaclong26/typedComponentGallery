...
export type DBTableProps1 = { path: string; bulkActions?: BulkAction[] } & (
  | ExpandProps
  | OpenProps
  | DefaultProps
);
...
export const DBTable: React.FC<DBTableProps> = ({
            ...
            <DropdownButton
              ...
              {bulkActions.map(({ name, function: actionFunction }, index) => (
                <Dropdown.Item
                  ...
                  onClick={() => {
                    if (docs) {
                      const selectedDocsData = Array.from(selectedDocs).map(
    ...
    return (
      <DropdownButton
        ...
        <Dropdown.Menu style={{ maxHeight: "200px", overflowY: "scroll" }}>
          {docs &&
            docs[0] &&
            Object.keys(docs[0].data).map((field, index) => (
              <Dropdown.Item
                key={index}
            ...
            {expand && ExpandComponent && expandedRowIndex === index && (
              <tr>
                <td
                  colSpan={
                    bulkActions
                      ? getColumnOrder().length + 1
                      : getColumnOrder().length
                  }>
  ...
  return (
    <>
      <Row className="my-3">
        {fields === "manual" && (
          <Col lg={1}>{renderFieldSelectionButton()}</Col>
        )}
