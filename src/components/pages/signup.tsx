...
const SignUp = () => {
  ...
  return (
    <Container className='fluid' style={{ position: "relative", paddingTop: '90px' }}>
      <Heading primary size={4} style={{ fontStyle: 'italic', position: 'absolute', top: '20px', left: '20px' }}>Sign Up</Heading>
      <img
        src={siteConfig.logo}
        width="50px"
      ...
      <Row className="mt-2">
        <Col lg={9} className="mx-auto text-center">
          {email && password && <Button rounded onClick={() => signUp(email!, password!)}>Sign Up</Button>}
        </Col>
      </Row>
      <Row className="mt-2">
        <Col lg={9} className="mx-auto text-center">
          <Button rounded onClick={signUpWithGoogle}>Sign up with Google</Button>
        </Col>
      </Row>
