...
import { EloiseWidget, Theme } from "../../";
import { UndefinedLogic, useEloise } from "../../App";
import { Logic } from "../../functions";
import { Color, HSLAColor } from "../../functions/color";
        ...
        backgroundColor={
          color
            ? typeof color === "number"
              ? logic.color.convertToHSLA(theme.colors[color])
              : color
            : logic.color.convertToHSLA(theme.colors[0])
        }
        theme={theme}
        rounded={rounded}
        className={className}
...
const ButtonWrapper = styled.button<ButtonWrapperProps>`
  ...
  background-color: ${(props) =>
    props.backgroundColor}; // Sets the background color to the passed color or default primary color
  color: white;
  border-radius: ${(props) =>
    props.rounded
      ? `3px`
